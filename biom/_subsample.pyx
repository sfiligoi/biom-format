...
cdef _subsample_slice_without_replacement(cnp.ndarray[TData, ndim=1] data,
                                          cnp.int32_t start,
                                          cnp.int32_t end,
                                          cnp.int64_t n,
                                          object rng):
...
cdef _subsample_fast_wo(cnp.ndarray[cnp.float64_t, ndim=1] data,
                        cnp.ndarray[cnp.int32_t, ndim=1] indptr,
                        cnp.int64_t n,
                        object rng):
    """Subsample non-zero values of a sparse array using an approximate method
    ...
    cdef:
        cnp.int64_t counts_sum, int_counts_sum
        cnp.int64_t rems_sum, int_counts_diff
        cnp.int32_t start,end,length
        Py_ssize_t i,j
        cnp.ndarray[cnp.int64_t, ndim=1] idata
    ...
    for i in range(indptr.shape[0] - 1):
        ...
        if (int_counts_diff>0):
            ...
            rems_sum = rems.sum()
            pvals = rems / rems_sum
            picked = rng.choice(length, int_counts_diff, replace=False, shuffle=False, p=pvals)
            for j in range(len(picked)):
                data[start+picked[j]] += 1
...
def _subsample(arr, n, with_replacement, rng, true_subsample):
    ...
    if (with_replacement):
       ...
       return _subsample_fast_wo(arr.data, arr.indptr, n, rng)
